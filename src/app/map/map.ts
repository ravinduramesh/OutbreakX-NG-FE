...
export class MapComponent implements AfterViewInit, OnDestroy {
  ...
  private createRoutingControl(waypoints: L.LatLng[]): void {
    ...
    this.routingControl = L.Routing.control({
      ...
      // @ts-ignore: createMarker is a valid option for leaflet-routing-machine but not in its typings
    } as any).addTo(this.map);

    // Set createMarker after control is created to avoid TypeScript error
    (this.routingControl as any).options.createMarker = (
      i: number,
      waypoint: L.LatLng,
      n: number
    ) => {
      // Custom markers for start (purple) and end (blue) of the route
      const iconUrl =
        i === 0
